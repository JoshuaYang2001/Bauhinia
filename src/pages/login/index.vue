    ...
    <div class="box">
        <div class="content">
            <div class="login-wrapper">
                <h1>Bauhinia-登录</h1>
                <h5 @click="byVisitor">以游客身份体验</h5>
                <div class="login-form">
                    <div class="username form-item">
                        <span>你的电子邮箱</span>
                        <input type="text" v-model="email" class="input-item">
                    </div>
                    <div class="password form-item">
                        <span>密码</span>
                        <input type="password" v-model="password" class="input-item">
                    </div>
                    <button class="login-btn" @click="login(email, password)">登 录</button>
                </div>
                <div class="divider">
                ...
                <div class="other-login-wrapper">
                    <div class="other-login-item" @click="byQQ">
                        <img src="./asset/QQ.png" alt="">
                    </div>
                    <div class="other-login-item" @click="byWeChat">
...
const byVisitor = () => {
    loginApi({ username: "visitor", password: "visitor" }).then(res => {
        setItem("userID", { "id": res.id, "nickName": "visitor" })
        router.push("/")
    }).catch(() => {
        alert('请稍后再试！');
    })
...
const login = (email: string, password: string) => {
    if (email === '' || password === '') {
        return alert("请完善登录信息！")
    }
    if (!IsEmail(email)) {
        return alert("请输入正确的邮箱！")
    }

    loginApi({ username: email, password }).then(res => {
        setItem("userID", { "id": res.id, "nickName": res.nickName })
        router.push("/")
    }).catch(() => {
        alert('用户名或密码错误');
    })
